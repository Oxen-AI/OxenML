...
for index in tqdm(range(total)):
  ...
  if correct_index == predicted_index and prob > threshold:
    print(f"TP {index}:{image_files[index]} -> {guessed_label} == {correct_label} {prob} > {threshold}%")
  ...
  # false positive
  if correct_index != predicted_index and prob > threshold:
    print(f"FP {index}:{image_files[index]} -> {guessed_label} != {correct_label} {prob} > {threshold}%")
  ...
  # true negative
  if correct_index != predicted_index and prob < threshold:
    print(f"TN {index}:{image_files[index]} -> {guessed_label} != {correct_label} {prob} < {threshold}%")
  ...
  # false negative
  if correct_index == predicted_index and prob < threshold:
    print(f"FN {index}:{image_files[index]} -> {guessed_label} == {correct_label} {prob} < {threshold}%")
