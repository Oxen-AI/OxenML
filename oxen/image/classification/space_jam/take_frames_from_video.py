...
output_dirs = [
    [relative_image_train_dir, image_train_dir],
    [relative_image_test_dir, image_test_dir],
]
filenames = []
key_counts = {}
    ...
    for (i, files) in enumerate(video_files):
        ...
        for file in files:
            ...
            if "first" == which_frames:
                filenames[i].extend(
                    take_first_frame(
                        file, relative_out_dir, output_dir, label, video_id
                    )
                )
            elif "first_mid_last" == which_frames:
                filenames[i].extend(
                    take_first_middle_last_frames(
                        file, relative_out_dir, output_dir, label, video_id
                    )
                )
            elif "all" == which_frames:
                filenames[i].extend(
                    take_all_frames(file, relative_out_dir, output_dir, label, video_id)
                )
