...
if len(sys.argv) != 5:
  print(f"Usage: {sys.argv[0]} <base-dir> <input-annotations.tsv> <output-dir> <total-per-cat>")
  exit()
...
output_annotations_file = os.path.join(output_annotations_dir, os.path.basename(annotations_file))
...
for key in total_label_counts.keys():
  ...
  if count < total_per_cat:
    print(f"Category `{key}` does not have enough examples {count} < {total_per_cat}")
    exit()
